...
export const MZPlusFlashOfferAdmin: React.FC = () => {
  ...
  const fetchConfig = async () => {
    try {
      const { data } = await supabase.from('mz_flash_offer_v2').select('*').eq('id', 'flash-offer-global').maybeSingle();
      if (data) {
  ...
  const fetchProofs = async () => {
    const { data } = await supabase.from('mz_premium_proofs').select('*').order('created_at', { ascending: false });
  ...
  return (
    <div className="space-y-12 animate-fade-in max-w-5xl mx-auto pb-20">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         <GoldBorderCard className="p-6 bg-black/40 border-purple-500/10 flex items-center gap-6">
            <div className="p-4 bg-purple-600/10 rounded-2xl text-purple-400 shadow-xl"><ImageIcon size={28} /></div>
            <div>
               <p className="text-[9px] font-black uppercase text-neutral-500 tracking-widest mb-1">Preuves enregistrées</p>
               <p className="text-3xl font-black text-white font-mono">{proofs.length}</p>
            </div>
         </GoldBorderCard>
         <GoldBorderCard className="p-6 bg-black/40 border-yellow-600/10 flex items-center gap-6">
            <div className="p-4 bg-yellow-600/10 rounded-2xl text-yellow-600 shadow-xl"><Zap size={28} /></div>
            <div>
               <p className="text-[9px] font-black uppercase text-neutral-500 tracking-widest mb-1">Status Offre</p>
               <p className={`text-sm font-black uppercase ${isActive ? 'text-emerald-500' : 'text-red-500'}`}>{isActive ? 'En ligne' : 'Désactivée'}</p>
            </div>
         </GoldBorderCard>
      </div>
      ...
      <GoldBorderCard className="p-8 border-yellow-600/20 bg-black/40">
        <div className="flex justify-between items-center mb-10 border-b border-white/5 pb-8">
           ...
           <div className="flex gap-3">
              <button onClick={() => setShowTimer(!showTimer)} className={`px-4 py-3 rounded-xl font-black uppercase text-[10px] flex items-center gap-2 ${showTimer ? 'bg-yellow-600 text-black' : 'bg-neutral-800 text-neutral-500'}`}>
                {showTimer ? <Timer size={14}/> : <TimerOff size={14}/>} {showTimer ? 'CHRONO ON' : 'CHRONO OFF'}
              </button>
              <button onClick={() => setIsActive(!isActive)} className={`px-6 py-3 rounded-xl font-black uppercase text-[10px] ${isActive ? 'bg-emerald-600 text-white shadow-[0_0_20px_rgba(16,185,129,0.4)]' : 'bg-red-600/20 text-red-500'}`}>
        ...
        <div className="space-y-8">
           ...
           <div className="p-6 bg-red-600/5 border border-red-500/10 rounded-2xl space-y-4">
              ...
              <div className="flex gap-4">
                <input 
                  type="number" 
                  className="w-32 bg-black border border-white/10 rounded-xl p-4 text-white outline-none focus:border-red-600 transition-all font-mono" 
                  value={urgencyHours} 
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center px-2">
          ...
          <button onClick={() => setShowProofForm(!showProofForm)} className="px-5 py-2.5 bg-white/5 border border-white/10 rounded-xl font-black uppercase text-[10px] hover:bg-white/10 transition-all">
            {showProofForm ? 'Fermer' : 'Nouvelle Preuve'}
          </button>
        </div>

        {showProofForm && (
          <GoldBorderCard className="p-8 bg-[#080808] border-purple-500/20 animate-slide-down">
            <form onSubmit={handleSaveProof} className="space-y-8">
               ...
               <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-4">
                     ...
                     <input required className="w-full bg-black border border-white/10 rounded-xl p-3 text-center text-[10px] text-white" placeholder="Gains (Ex: 0 CFA)" value={proofForm.before_amount} onChange={e => setProofForm({...proofForm, before_amount: e.target.value})} />
                  ...
                  <div className="space-y-4">
                     ...
                     <input required className="w-full bg-black border border-white/10 rounded-xl p-3 text-center text-[10px] text-white" placeholder="Gains (Ex: 95 000 CFA)" value={proofForm.after_amount} onChange={e => setProofForm({...proofForm, after_amount: e.target.value})} />
          ...
          {proofs.map(p => (
            <div key={p.id} className="p-4 bg-neutral-900/40 border border-white/5 rounded-2xl flex items-center justify-between group hover:border-purple-500/20 transition-all">
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="min-w-0"><p className="text-[10px] font-black uppercase text-white truncate">{p.name}</p><p className="text-[8px] text-neutral-600 uppercase font-bold">{p.after_amount}</p></div>
              </div>
