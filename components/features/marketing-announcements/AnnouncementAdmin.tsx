...
const REDIRECT_TABS = [
  ...
  { id: 'flash_offer', label: 'Offre Flash MZ+' },
...
export const AnnouncementAdmin: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex justify-between items-center px-2">
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="p-3 bg-neutral-900 border border-white/5 rounded-xl text-neutral-500 hover:text-yellow-600 transition-all active:rotate-180"
            title="Rafraîchir les compteurs"
          >
            <RefreshCw size={16} />
          </button>
          <button onClick={() => showForm ? resetForm() : setShowForm(true)} className="flex items-center gap-2 px-6 py-3 bg-yellow-600 text-black rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-yellow-500 transition-all shadow-lg active:scale-95">
      ...
      {showForm && (
        <GoldBorderCard className="p-8 md:p-12 border-yellow-600/20 bg-black/40 animate-slide-down overflow-visible">
          {errorMsg && (
          ...
          {successMsg ? (
            ...
            <form onSubmit={handleSave} className="space-y-10">
              ...
              <PrimaryButton type="submit" fullWidth isLoading={isSaving} size="lg">Déclencher le Pop-up sur le réseau</PrimaryButton>
            </form>
          )}
        </GoldBorderCard>
      )}
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="py-20 text-center border border-dashed border-white/5 rounded-[2.5rem] opacity-20"><Megaphone size={40} className="mx-auto mb-4" /><p className="text-[10px] font-black uppercase">Aucune annonce</p></div>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            {announcements.map(ann => (
              <div key={ann.id} className="p-6 bg-[#0a0a0a] border border-white/5 rounded-[2rem] flex items-center justify-between group hover:border-yellow-600/20 transition-all">
                <div className="flex items-center gap-6">
                  <div className="w-12 h-12 bg-neutral-900 rounded-xl flex items-center justify-center text-neutral-500"><FileText size={20} /></div>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => toggleActive(ann.id, ann.is_active)} className={`p-3 rounded-xl transition-all ${ann.is_active ? 'bg-emerald-600/10 text-emerald-500 border border-emerald-500/20' : 'bg-red-600/10 text-red-500 border border-red-500/20'}`} title={ann.is_active ? "Désactiver" : "Activer"}><Power size={16} /></button>
                  <button onClick={() => deleteAnnouncement(ann.id)} className="p-3 bg-white/5 text-neutral-600 hover:text-red-500 border border-white/5 rounded-xl transition-all"><Trash2 size={16} /></button>
                </div>
              </div>
            ))}
          </div>
