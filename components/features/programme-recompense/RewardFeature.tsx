...
export const RewardFeature: React.FC<{ profile: UserProfile | null }> = ({ profile }) => {
    ...
    try {
      ...
      if (data) {
        ...
        if (me) setMyRank(me);
            ...
            const { data: individual } = await supabase.from('mz_rewards_leaderboard_v2').select('*').eq('user_id', profile.id).single();
           ...
           <div className={`p-8 rounded-[2.5rem] border-2 transition-all duration-700 relative overflow-hidden group ${isEligibleLevel ? 'bg-emerald-500/5 border-emerald-500/30' : 'bg-red-500/5 border-red-500/20'}`}>
              ...
              <div className="space-y-2">
                 ...
                 <p className="text-[11px] text-neutral-500 font-medium leading-relaxed uppercase">
                    {isEligibleLevel 
                      ? "Félicitations, ton accès Premium est activé. Tu es prêt à encaisser." 
