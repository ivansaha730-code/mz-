...
export const AdminActivityAudit: React.FC = () => {
  ...
  useEffect(() => {
    fetchAudit();
    const interval = setInterval(fetchAudit, 120000); 
              ...
              return (
                <div key={user.user_id} className={`transition-all duration-500 ${isExpanded ? 'scale-[1.01]' : 'scale-100'}`}>
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-5 w-full lg:w-auto">
                        <div className={`relative shrink-0`}>
                          <div className={`w-14 h-14 md:w-16 md:h-16 rounded-[1.8rem] flex items-center justify-center font-black uppercase text-xl border-2 transition-all duration-700 ${
                          ...
                          {isOnline && (
                            <div className="absolute -top-1 -right-1 w-5 h-5 bg-emerald-500 rounded-full border-4 border-[#0c0c0c] animate-pulse"></div>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-3">
                            <h4 className="text-base md:text-lg font-black uppercase text-white truncate tracking-tight">{user.full_name}</h4>
                            {/* Fix: EliteBadge is now correctly imported */}
                            <EliteBadge variant={user.user_level as any}>{user.user_level === 'niveau_mz_plus' ? 'MZ+' : 'STD'}</EliteBadge>
                          </div>
                          <p className="text-[11px] text-neutral-500 font-mono truncate mt-1">{user.email}</p>
                        </div>
                      </div>

                      <div className="grid grid-cols-3 gap-8 md:gap-16 w-full lg:w-auto pt-6 lg:pt-0 border-t lg:border-t-0 border-white/5">
                        ...
                        <div className="text-center sm:text-left flex flex-col justify-center">
                           <p className="text-[8px] font-black uppercase text-neutral-600 tracking-widest mb-1.5">Statut</p>
                           <p className={`text-[10px] font-black uppercase tracking-widest ${isOnline ? 'text-emerald-500' : 'text-neutral-800'}`}>
                             {isOnline ? 'Active Session' : 'Disconnected'}
                           </p>
                        </div>
                      </div>
                      ...
                      <div className="hidden lg:flex flex-col items-end min-w-[120px]">
                         <div className={`p-2 rounded-xl border flex items-center gap-2 ${isOnline ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-white/5 border-white/5 text-neutral-700'}`}>
                           ...
                           <span className="text-[9px] font-black uppercase">{isOnline ? 'Live Audit' : 'Offline'}</span>
                         </div>
                         {user.last_active && (
                    ...
                    {isExpanded && (
                      <div className="bg-black/60 border-t border-white/5 p-8 md:p-12 animate-fade-in relative">
                         <div className="flex items-center justify-between mb-10">
                            <div className="flex items-center gap-4">
                              <div className="p-3 bg-yellow-600/10 rounded-2xl text-yellow-600">
                                <History size={20} />
                              </div>
                              <h5 className="text-[12px] font-black uppercase tracking-[0.3em] text-white">Rapport d'Activité <GoldText>14 Jours</GoldText></h5>
                            </div>
                            <p className="text-[9px] font-black uppercase text-neutral-600 italic">Mise à jour: {new Date().toLocaleTimeString()}</p>
                         </div>
                         ...
                         {loadingHistory ? (
                           ...
                           <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-6 items-end">
                              {userHistory.map((day, idx) => {
                                ...
                                return (
                                  <div key={idx} className="flex flex-col items-center gap-4 group/bar">
                                    <div className="w-full bg-white/[0.03] rounded-t-xl overflow-hidden relative flex flex-col justify-end h-32 md:h-40 border border-white/5 group-hover/bar:border-yellow-600/30 transition-all">
                                       <div 
                                       ...
                                       <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/bar:opacity-100 transition-opacity">
                                          <span className="bg-black/80 px-2 py-1 rounded text-[8px] font-black text-yellow-500 border border-yellow-600/30">{day.total_minutes}m</span>
                                       </div>
                                    </div>
                                    <div className="text-center">
                                      <p className="text-[8px] font-black text-neutral-400 uppercase mb-1">
                                        {new Date(day.tracking_date).toLocaleDateString('fr-FR', { weekday: 'short' })}
                                      </p>
                                      <p className={`text-[7px] font-black font-mono uppercase tracking-tighter ${day.total_minutes > 0 ? 'text-white' : 'text-neutral-800'}`}>
