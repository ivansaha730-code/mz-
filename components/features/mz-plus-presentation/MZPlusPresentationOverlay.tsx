...
export const MZPlusPresentationOverlay: React.FC<{ profile: any; onUpgrade: () => void }> = ({ profile, onUpgrade }) => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      const { data } = await supabase.from('mz_plus_offer_config').select('*').single();
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black overflow-y-auto animate-fade-in custom-scrollbar">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 space-y-16">
        ...
        {/* Video Section */}
        <div className="relative aspect-video rounded-[2.5rem] overflow-hidden border border-purple-600/30 shadow-[0_0_100px_rgba(139,92,246,0.15)] bg-neutral-900 group">
          <iframe 
            className="w-full h-full"
            src={`https://www.youtube.com/embed/${config.youtube_id}?autoplay=1&rel=0&mute=0&controls=1`}
            title="MZ+ Presentation"
        ...
        <div className="bg-gradient-to-br from-[#0e002a] to-black border border-purple-600/20 rounded-[3rem] p-8 md:p-12 text-center space-y-8 relative overflow-hidden">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col items-center gap-2">
              <span className="text-neutral-600 text-2xl line-through font-black">150 000 CFA</span>
              <div className="flex items-baseline gap-3">
                <span className="text-6xl md:text-8xl font-black text-white tracking-tighter font-mono">20 000</span>
                <span className="text-xl md:text-2xl text-purple-500 font-black uppercase">CFA</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-10">
          ...
          <FeatureItem title="Formation Millionnaire" desc="Le pack complet pour générer 1M/mois avec l'IA et TikTok." />
