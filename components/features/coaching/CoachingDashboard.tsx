  ...
  const steps = [
    {
      ...
      content: (
        <div className="grid grid-cols-1 gap-3">
          {[
            { id: "100k", label: "Mes premiers 100 000 CFA", sub: "Lancer la machine" },
            { id: "500k", label: "Atteindre 500 000 CFA / mois", sub: "Liberté naissante" },
            { id: "1M", label: "Viser le Million et plus", sub: "Statut Ambassadeur Élite" }
          ].map((opt) => (
            <button
              key={opt.id}
            ...
            >
              <div className="flex justify-between items-center">
                 <div>
                    <p className={`text-[11px] font-black uppercase tracking-widest ${formData.objective === opt.label ? 'text-white' : 'text-neutral-400'}`}>{opt.label}</p>
