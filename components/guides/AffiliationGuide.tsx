...
export const AffiliationGuide: React.FC<AffiliationGuideProps> = ({ isActive, onComplete, activeCategory, activeTab }) => {
  ...
  const getTargetId = () => {
    ...
    if (step === 10) return null; // Community panel
    return null;
  };
  ...
  useEffect(() => {
    ...
    if (isActive) {
      if (targetId) {
        ...
        // Step 10 (Community) appears after 5 seconds as requested
        const panelDelay = step === 10 ? 5000 : 900;
        activeTimer = window.setTimeout(() => {
          setIsReady(true);
        }, panelDelay);
      }
  ...
  const messages = [
    ...
    "👉 Intègre le groupe WhatsApp pour recevoir conseils et stratégies pour vendre plus."
  ];
  ...
  return (
    <>
      ...
      <GuideStep 
        ...
        title={step === 10 ? "🚀 Rejoins la communauté" : undefined}
        link={step === 10 ? { label: "Rejoindre le groupe WhatsApp", url: "https://chat.whatsapp.com/CyC2EYXWMnx8WteZtZ4s8s" } : undefined}
        targetRect={targetRect}
        variant={(step === 1 || step === 3 || step === 6 || step === 7 || step === 9 || step === 10) ? 'panel' : 'tooltip'}
